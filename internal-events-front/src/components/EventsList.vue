<template>
  <v-container class="blue lighten-5">
    <v-row class="list px-12 dense">
      <v-col v-for="event in events" :key="event.id" class="d-flex">
        <!-- "tile" card's form-->
        <v-card class="d-flex flex-column" width="300px" height="100%" tile>
          <v-card-text>
            <div class="overline mb-1">{{ event.nameEventType }}</div>

            <p class="title text--primary">{{ event.title }}</p>
            <p class="subtitle-1 ">
              {{ formatDateTime(event.startDateTime) }} <br />
              <span class="subtitle-2"
                >{{ event.nameBuilding }} <br />
                <div v-if="event.nameRoom === 'N/A'"></div>
                <div v-else>Salle {{ event.nameRoom }}</div></span
              >
            </p>
          </v-card-text>
          <v-card-actions>
            <router-link :to="{ name: 'eventDetails', params: { id : event.id } }">
          En savoir plus
            </router-link>
          </v-card-actions>
        </v-card>
      </v-col>
    </v-row>
  </v-container>
</template>

<script>
import moment from "moment";

export default {
  name: "events-list",
  data() {
    return {
      //
    };
  },
  computed: {
    events() {
      return this.$store.state.event.events;
    },
  },
  methods: {
      formatDateTime(dateTime) {
      moment.locale("fr");
      return moment(dateTime).format("ddd ll [à] LT");
    },
  },

  mounted() {
    this.$store.dispatch("event/getEvents");
  },
};
</script>

<style>
.list {
  max-width: 100%;
}
</style>
